# Comparatif mobile Fondateur — direction visuelle HORIZON validée

**Décision du 25 septembre 2026 :** la direction visuelle HORIZON est validée pour le seul compte principal Fondateur. Les raccourcis et la navigation de la fiche client, représentés dans `horizon-parcours-fondateur.html`, restent des propositions non validées ; aucune intégration n'est autorisée par ce seul choix esthétique. Les comptes standard gardent Clarté 01.
...
**Recommandation de parcours (à confirmer) :** après validation du langage visuel HORIZON, proposer deux raccourcis directs de la Tour de contrôle sur la fiche client (Nouvel équipement et Inviter un membre). Laisser la palette aux opérations moins fréquentes, garder Support et le scanner QR en accès rapide, le nom du client visible avant chaque action, et les quatre onglets Fiche/Parc/Équipe/Invitations. Cette piste est dessinée dans `horizon-parcours-fondateur.html` ; ne pas l'intégrer sans validation explicite de l'utilisateur.
